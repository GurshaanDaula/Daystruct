...
function addDropzoneEvents(timeSlot, dropzone) {
  ...
  dropzone.addEventListener("drop", (e) => {
    ...
    const alreadyExists = schedules[currentDay][period].some(
      (a) => a.id === activity.id,
    );
...
assistantBtn.addEventListener("click", () => {
  ...
  if (!prompt) {
    assistantResponse.textContent =
      "Try asking for help with productivity, focus, consistency, or balance.";
    return;
  }

  assistantResponse.textContent = `Suggestion: Based on "${prompt}", try putting your hardest task first, leaving a recovery block after it, and grouping similar activities together to reduce mental switching.`;
});
